...
const TEACHER_PRICE_ID = Deno.env.get("STRIPE_TEACHER_PRICE_ID") || "price_teacher";
const STUDENT_PRICE_ID = Deno.env.get("STRIPE_STUDENT_PRICE_ID") || "price_student";
serve(async (req)=>{
  ...
  try {
    ...
    try {
      const { data: existingSubscription, error: checkError } = await fetch(`${Deno.env.get("SUPABASE_URL")}/rest/v1/subscriptions?school_id=eq.${schoolId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      if (!existingSubscription || existingSubscription.length === 0) {
        console.log(`No existing subscription found for school ${schoolId}, creating initial record...`);
        const initSubscription = await fetch(`${Deno.env.get("SUPABASE_URL")}/rest/v1/subscriptions`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            apikey: Deno.env.get("SUPABASE_ANON_KEY") || "",
        ...
        console.log(`Initialized subscription record: ${await initSubscription.text()}`);
      } else {
        console.log(`Found existing subscription for school ${schoolId}`);
      }
    } catch (dbError) {
      console.error(`Error initializing subscription record: ${dbError.message}`);
    ...
    console.log(`Created checkout session: ${session.id} for schoolId: ${schoolId}`);
