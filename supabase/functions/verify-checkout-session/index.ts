...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Retrieved session from Stripe: ${JSON.stringify(session, null, 2)}`,
    );
    ...
    if (!schoolId) {
      return new Response(
        JSON.stringify({ success: false, error: "No school ID in metadata" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (
      ...
      console.log(
        `Subscription missing Stripe IDs. Updating for school: ${schoolId}`,
      );
      ...
      console.log(
        `Subscription already has Stripe IDs for school: ${schoolId}`,
      );
    }
