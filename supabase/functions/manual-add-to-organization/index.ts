...
serve(async (req)=>{
  ...
  try {
    ...
    if (diagnosticRequest) {
      ...
      if (diagnosticResults.apiTests.specificUser?.success && diagnosticResults.apiTests.specificOrganization?.success) {
        try {
    ...
    try {
      ...
      const clerkRole = role.toLowerCase() === "admin" ? "org:admin" : role.toLowerCase();
      console.log(`Using standardized role format for Clerk API: '${clerkRole}'`);
      ...
      if (!response.ok) {
        ...
        if (errorData.errors && errorData.errors[0]?.meta?.param_name === "role") {
          console.error(`Role parameter '${clerkRole}' is incorrect. Clerk API rejected this role format.`);
          console.error(`Available roles in Clerk: 'org:admin', 'basic_member'`);
          // Return a more informative error response
          return new Response(JSON.stringify({
            error: `Invalid role format: ${clerkRole}`,
            details: errorData,
            suggestedAction: "Check Clerk documentation for valid role formats",
            validRoles: [
              "org:admin",
              "basic_member"
            ]
      ...
      let verified = false;
      try {
        const verifyMemberships = await clerk.users.getOrganizationMembershipList({
          userId
        });
