...
console.log(
  `Running in environment: ${Deno.env.get("SUPABASE_ENV") || "unknown"}`,
);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Payload received: organizationId=${organizationId}, userId=${userId}, role=${role}, diagnosticRequest=${diagnosticRequest}`,
    );
    ...
    if (diagnosticRequest) {
      ...
      try {
        ...
        diagnosticResults.errors.push(
          `Organization listing failed: ${error.message}`,
        );
      }
      ...
      if (organizationId) {
        try {
          ...
          diagnosticResults.errors.push(
            `Specific organization (${organizationId}) retrieval failed: ${error.message}`,
          );
        }
      }
      ...
      if (userId) {
        try {
          ...
          diagnosticResults.errors.push(
            `Specific user (${userId}) retrieval failed: ${error.message}`,
          );
        }
      }
      ...
      if (
        diagnosticResults.apiTests.specificUser?.success &&
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          ...
          details: orgError,
          suggestedAction:
            "Verify this organization ID exists and is accessible with your API key",
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    try {
      ...
      const user = await clerk.users.getUser(userId);
      console.log(
        `User verified: ${user.emailAddresses[0]?.emailAddress || "unknown email"}`,
      );
    } catch (userError) {
      ...
      return new Response(
        JSON.stringify({
          ...
          details: userError,
          suggestedAction:
            "Verify this user ID exists and is accessible with your API key",
        }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    try {
      ...
      if (existingMembership) {
        console.log(
          `User is already a member with role: ${existingMembership.role}`,
        );
        ...
        try {
          console.log(
            `Updating membership role from ${existingMembership.role} to ${role}`,
          );
          ...
          return new Response(
            JSON.stringify({
              ...
              details: updateError,
              suggestedAction:
                "Verify that the API key has permissions to update membership roles",
            }),
            {
              status: 500,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    try {
      console.log(
        `Adding user ${userId} to organization ${organizationId} with role ${role}`,
      );

      // Log request payload for createOrganizationMembership
      console.log(
        `createOrganizationMembership payload: { organizationId: ${organizationId}, userId: ${userId}, role: ${role} }`,
      );

      // Determine the correct role format based on Clerk's API requirements
      // Clerk API expects 'admin' for basic role and 'org:admin' for organization admin role
      const clerkRole =
        role.toLowerCase() === "admin" ? "org:admin" : role.toLowerCase();
      console.log(
        `Using standardized role format for Clerk API: '${clerkRole}'`,
      );

      // Using a direct API call with the correct role format from the start
      const response = await fetch(
        `https://api.clerk.dev/v1/organizations/${organizationId}/memberships`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        // If we get a specific error about the role, provide more detailed diagnostics
        if (
          errorData.errors &&
          errorData.errors[0]?.meta?.param_name === "role"
        ) {
          console.error(
            `Role parameter '${clerkRole}' is incorrect. Clerk API rejected this role format.`,
          );
          console.error(
            `Available roles in Clerk: 'org:admin', 'basic_member'`,
          );

          // Return a more informative error response
          return new Response(
            JSON.stringify({
              error: `Invalid role format: ${clerkRole}`,
              details: errorData,
              suggestedAction:
                "Check Clerk documentation for valid role formats",
              validRoles: ["org:admin", "basic_member"],
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        if (response.status === 404) {
          return new Response(
            JSON.stringify({
              ...
              details: errorData,
              suggestedAction:
                "Verify that both organization and user IDs exist and are accessible with your API key",
              apiErrorDetails: {
      ...
      let verified = false;
      try {
        const verifyMemberships =
          await clerk.users.getOrganizationMembershipList({ userId });
        verified = verifyMemberships.some(
          (m) => m.organization.id === organizationId,
        );
        console.log(
          `Membership verification: ${verified ? "successful" : "failed"}`,
        );
      } catch (verifyError) {
      ...
      return new Response(
        JSON.stringify({
          ...
          details: error,
          suggestedAction:
            "Check API key permissions for creating organization memberships",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.error(
      `Unhandled error in manual-add-to-organization: ${error.message}`,
    );
