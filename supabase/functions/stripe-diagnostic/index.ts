...
serve(async (req)=>{
  ...
  try {
    ...
    console.log(`Diagnostic request: action=${action}, schoolId=${schoolId}, sessionId=${sessionId}, subscriptionId=${subscriptionId}`);
    let result = {};
    switch(action){
      ...
      case "fix-missing-stripe-ids":
        if (!schoolId || !sessionId) {
          throw new Error("schoolId and sessionId are required for this action");
        }
...
async function checkEnvironment() {
  ...
  for (const varName of envVars){
    ...
    results[varName] = {
      ...
      preview: value ? `${value.substring(0, 4)}...${value.substring(value.length - 4)}` : "",
      length: value.length
    };
  }
  return {
...
async function checkSubscriptionTable() {
  try {
    ...
    const { data: recentSubs, error: recentError } = await supabase.from("subscriptions").select("*").order("updated_at", {
      ascending: false
    }).limit(3);
    if (recentError) {
      return {
    ...
    const { data: missingIds, error: missingError } = await supabase.from("subscriptions").select("id, school_id, status, updated_at").or("stripe_customer_id.is.null,stripe_subscription_id.is.null").eq("status", "active").order("updated_at", {
      ascending: false
    }).limit(5);
    return {
      subscriptionTable: {
...
async function getSchoolSubscription(schoolId) {
  try {
    ...
    if (data.stripe_subscription_id) {
      try {
        stripeSubscription = await stripe.subscriptions.retrieve(data.stripe_subscription_id);
      } catch (stripeError) {
  ...
  try {
    ...
    if (!session.subscription || !session.customer) {
      return {
        fixResult: {
          success: false,
          message: "Checkout session does not have subscription or customer IDs",
          session
        }
      };
    }
