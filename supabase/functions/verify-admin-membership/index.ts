...
const isValidId = (id: string, prefix: string): boolean => {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Payload received: organizationId=${organizationId}, userId=${userId || "not provided"}`,
    );
    ...
    try {
      ...
      console.log(
        `User verified: ${user.emailAddresses[0]?.emailAddress || "email not found"}`,
      );
    } catch (userError) {
    ...
    try {
      ...
      console.log(
        `Found ${memberships.length} organization memberships for user`,
      );
      ...
      if (existingMembership) {
        ...
        if (!isAdmin) {
          try {
            console.log(
              `Promoting user from ${existingMembership.role} to admin role`,
            );

            // First, log detailed information about the membership for debugging
            console.log(
              `Membership details before promotion: ID=${existingMembership.id}, Role=${existingMembership.role}`,
            );
            console.log(
            ...
            const response = await fetch(
              `https://api.clerk.dev/v1/organization_memberships/${existingMembership.id}`,
              {
                method: "PATCH",
                headers: {
            ...
            if (response.ok) {
              ...
              isAdmin = true;
              console.log(
                `Successfully promoted user to admin role. New role: ${updatedMembership.role}`,
              );
            } else {
              const errorData = await response.json();
              console.error(
                `Direct API call for promotion failed with status ${response.status}`,
              );
              ...
              isAdmin = true;
              console.log(
                `Successfully promoted user to admin role using SDK fallback`,
              );
            }
          } catch (promoteError) {
        ...
        console.log(
          `User is not a member of this organization, will add as admin`,
        );
        ...
        try {
          ...
          isAdmin = true;
          console.log(
            `Successfully added user as admin. Membership ID: ${newMembership.id}`,
          );

          // Verify membership was actually created
          const updatedMemberships =
            await clerk.users.getOrganizationMembershipList({ userId });
          const verifyMembership = updatedMemberships.find(
            (m) => m.organization.id === organizationId,
          );

          if (!verifyMembership) {
            console.error(
              `Membership verification failed: membership not found after creation`,
            );
            wasAdded = false;
            isAdmin = false;
            errors.push(
              "Membership creation appeared to succeed but verification failed",
            );
          }
        } catch (addError) {
          ...
          try {
            console.log(
              "Attempting alternative membership creation approach...",
            );
      ...
      console.log(
        `Final membership status: isAlreadyMember=${isAlreadyMember}, isAdmin=${isAdmin}, wasPromoted=${wasPromoted}, wasAdded=${wasAdded}`,
      );
    } catch (error) {
    ...
    const responseStatus =
      isAdmin && (isAlreadyMember || wasAdded || wasPromoted) ? 200 : 207;
    ...
    console.error(
      `Unhandled error in verify-admin-membership function: ${error.message}`,
    );
