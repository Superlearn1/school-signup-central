...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `STRIPE_TEACHER_PRICE_ID set: ${!!Deno.env.get("STRIPE_TEACHER_PRICE_ID")}`,
    );
    console.log(
      `STRIPE_STUDENT_PRICE_ID set: ${!!Deno.env.get("STRIPE_STUDENT_PRICE_ID")}`,
    );
    ...
    try {
      ...
      console.log(
        `Stripe connection test: Success. Found ${testProduct.data.length} products`,
      );
    } catch (stripeError) {
