...
export const createClerkOrganization = async (
  ...
  try {
    console.log(
      `Attempting to create Clerk organization "${name}" for school ID: ${schoolId} with admin: ${adminUserId}`,
    );
    ...
    if (!data || !data.id) {
      console.error(
        "Invalid response from create-organization function:",
        data,
      );
    ...
    if (data.adminAdded === false) {
      console.warn(
        "Warning: Organization created but admin was not added:",
        data.warning,
      );
      ...
      const scheduleRetries = async () => {
        ...
        for (let i = 0; i < retryTimes.length; i++) {
          ...
          try {
            console.log(
              `Retry ${i + 1}/${retryTimes.length} to fix admin membership after ${retryTimes[i]}ms delay...`,
            );
            const success = await checkAndFixOrganizationAdmin(
              data.id,
              adminUserId,
            );
            if (success) {
              console.log(
                `Successfully fixed admin membership on retry ${i + 1}`,
              );
              break;
            }
          } catch (retryError) {
            console.error(
              `Failed to fix admin membership on retry ${i + 1}:`,
              retryError,
            );
          }
        }
      ...
      console.warn(
        "Warning: Admin added but verification failed:",
        data.warning,
      );
      ...
      setTimeout(async () => {
        try {
          ...
          console.error(
            "Failed to verify admin membership after delay:",
            fixError,
          );
        }
    ...
    setTimeout(async () => {
      try {
        const { data: membershipData } = await supabase.functions.invoke(
...
export const checkAndFixOrganizationAdmin = async (
  ...
  console.log(
    `[${requestId}] Checking admin membership for org: ${organizationId}, user: ${userId || "current user"}`,
  );
  ...
  try {
    ...
    if (
      ...
      verifyData.error !== "User is already a member" &&
      userId
    ) {
      console.warn(
        `[${requestId}] Verification failed, attempting manual add. Error: ${verifyData.error}`,
      );
      return await manuallyAddUserToOrganization(
        organizationId,
        userId,
        "admin",
        requestId,
    ...
    console.log(
      `[${requestId}] Falling back to original implementation via supabase client`,
    );
    const { data, error } = await supabase.functions.invoke(
      "verify-admin-membership",
      {
    ...
    if (error) {
      ...
      if (
        ...
        console.error(
          `[${requestId}] Organization not found. It may not have propagated yet or was deleted`,
        );
      }
      ...
      if (userId) {
        try {
          console.warn(
            `[${requestId}] Error during verification, attempting manual add as last resort`,
          );
          return await manuallyAddUserToOrganization(
            organizationId,
            userId,
          ...
          console.error(
            `[${requestId}] Manual add failed as well:`,
            manualError,
          );
        }
      }
    ...
    console.error(
      `[${requestId}] Error in checkAndFixOrganizationAdmin:`,
      error,
    );
...
/**
 * Attempts to verify and fix organization admin membership with optimized retry strategy
...
export const verifyOrganizationAdminWithRetry = async (
  organizationId: string,
  userId?: string,
  maxAttempts = 2, // Keep reduced attempts to minimize waiting
  initialBackoff = 1000, // Further reduced initial backoff
): Promise<boolean> => {
  // Generate a unique request ID for tracking this verification attempt
  const requestId = `verify_${Math.random().toString(36).substring(2, 10)}`;
  console.log(
    `[${requestId}] Starting organization admin verification for org: ${organizationId}, user: ${userId || "current user"}`,
  );

  // First, check if the user is already a member and their role
  try {
    console.log(`[${requestId}] Checking current membership status`);

    // Use direct fetch for more control and better error handling
    const membershipCheckResponse = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/verify-admin-membership`,
      {
        ...
        body: JSON.stringify({
          ...
          userId,
          requestId,
          diagnosticMode: true, // Request additional diagnostic info
        }),
      },
    );
    ...
    if (membershipCheckResponse.ok) {
      ...
      console.log(
        `[${requestId}] Initial membership check result:`,
        membershipData,
      );
      ...
      if (membershipData.success && membershipData.isAdmin) {
        console.log(
          `[${requestId}] User is already an admin, no further action needed`,
        );
        return true;
      }

      // If already a member but not admin, try a direct promotion
      if (membershipData.alreadyMember && !membershipData.isAdmin) {
        console.log(
          `[${requestId}] User is a member but not admin, attempting direct promotion`,
        );

        // Try manual add with explicit org:admin role
        const promotionResult = await manuallyAddUserToOrganization(
          organizationId,
          userId || "",
          "org:admin", // Use explicit org:admin role
          `${requestId}_promote`,
        );

        if (promotionResult) {
          console.log(`[${requestId}] Successfully promoted user to admin`);
          return true;
        }

        console.warn(
          `[${requestId}] Failed to promote user, will try standard verification`,
        );
      }
    } else {
      console.warn(
        `[${requestId}] Initial membership check failed, status: ${membershipCheckResponse.status}`,
      );
    }
  } catch (error) {
    console.warn(
      `[${requestId}] Error during initial membership check:`,
      error,
    );
  }

  // If initial checks fail, proceed with limited retries of the standard verification
  let attempts = 0;
  let success = false;
  ...
  while (!success && attempts < maxAttempts) {
    attempts++;
    console.log(
      `[${requestId}] Attempt ${attempts}/${maxAttempts} to verify organization admin membership`,
    );
    ...
    try {
      ...
      if (success) {
        console.log(
          `[${requestId}] Successfully verified admin membership on attempt ${attempts}`,
        );
        return true;
      }
    } catch (error) {
      console.warn(
        `[${requestId}] Verification attempt ${attempts} failed:`,
        error,
      );
    }

    if (attempts < maxAttempts) {
      // Shorter backoff to reduce waiting time
      backoffDelay = Math.min(backoffDelay * 1.2, 2000); // Cap at 2 seconds
      console.log(
        `[${requestId}] Verification failed, waiting ${Math.round(backoffDelay)}ms before retry...`,
      );
  ...
  console.error(
    `[${requestId}] Failed to verify admin membership after ${attempts} attempts`,
  );
  return false;
};
...
export const manuallyAddUserToOrganization = async (
  ...
  try {
    console.log(
      `[${requestId}] Manually adding user ${userId} to organization ${organizationId} with role ${role}`,
    );
    ...
    console.error(
      `[${requestId}] Error in manuallyAddUserToOrganization:`,
      error,
    );
    return false;
  }
...
export const createOrganization = async (
  ...
  console.log(
    `[${requestId}] Creating organization: ${name}, slug: ${slug}, adminUserId: ${adminUserId}`,
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to create organization: ${errorData.error || response.statusText}`,
      );
    }
    ...
    if (data.warnings?.adminNotAdded) {
      console.warn(
        `[${requestId}] Admin not added to organization initially, will attempt to fix`,
      );

      // Immediately try to fix the admin membership
      try {
        await checkAndFixOrganizationAdmin(
        ...
        console.error(
          `[${requestId}] Initial attempt to fix admin membership failed:`,
          fixError,
        );
      ...
      retryDelays.forEach((delay, index) => {
        setTimeout(async () => {
          try {
            console.log(
              `[${requestId}] Retry #${index + 1} to verify admin membership after ${delay}ms`,
            );
            await checkAndFixOrganizationAdmin(
              data.organization.id,
              adminUserId,
              `${requestId}_retry${index + 1}`,
            );
          } catch (retryError) {
            console.error(
              `[${requestId}] Retry #${index + 1} failed:`,
              retryError,
            );
          }
        }, delay);
      ...
      setTimeout(async () => {
        try {
          console.log(
            `[${requestId}] Final verification of admin membership after all retries`,
          );
          // Fix: Use supabase function to check membership instead of Clerk hooks
          const { data: membershipData, error: membershipError } =
            await supabase.functions.invoke("verify-admin-membership", {
              body: {
                organizationId: data.organization.id,
                userId: adminUserId,
                requestId: `${requestId}_final`,
              },
            });

          if (membershipError) {
            console.error(
              `[${requestId}] Final membership check failed:`,
              membershipError,
            );
          } else {
            console.log(
              `[${requestId}] Admin membership status after retries:`,
